...
export default async function ProductPage({ params }: { params: Params }) {
  ...
  return (
    <section className="px-5 lg:px-52 py-12 space-y-10 ">
      <Link
      ...
      <div className="grid grid-cols-3 md:grid-cols-2 gap-12 mt-5">
        ...
        <div className="space-y-5 flex flex-col justify-center md:col-span-1 col-span-2 ">
          ...
          <h1 className="md:text-5xl text-2xl font-bold uppercase">{product.name}</h1>
          <p className="text-gray-500">{product.description}</p>
          <p className="text-xl">$ {product.price.toLocaleString()}</p>
        </div>
      </div>
      <div className="grid md:grid-cols-3 md:gap-16 gap-7 pb-16 ">
        ...
        <div className="flex lg:flex-col flex-row gap-5">
            ...
            <ul>
                {product.includedItems.map((item)=>(
                    <li key={item.item} className="text-neutral-500"><span className="text-[#d87d4a]"> {item.quantity}X</span> {item.item}</li>
                ))}
            </ul>
        </div>
